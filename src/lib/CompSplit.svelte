<script lang="ts">
    ...
    type Length = `${number}px` | `${number}%` | `${number}em` | `${number}rem`;
...
<svelte-split-pane
    ...
    style="--pos: {normalize(current)}; --min: {normalize(
        min,
    )}; --max: {normalize(max)}"
>
    <svelte-split-pane-section
        bind:contentRect={
            ...
            (rect: DOMRect) => {
                ...
                const parent_size =
                    type === "columns" ? parent.width : parent.height;
                ...
                if (pos.endsWith("%")) {
                    ...
                    if (pos.endsWith("px")) {
                        ...
                        const font_size = parseFloat(
                            getComputedStyle(
                                is_rem ? document.documentElement : container,
                            ).fontSize,
                        );
